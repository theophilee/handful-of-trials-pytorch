...
class Experiment:
    ...
    def run_behavior_cloning(self):
        ...
        for i in range(self.num_rollouts):
            ...
            # TODO what task horizon to use under approximate model? are full rollouts useful?
            obs_imagined, acts_imagined = self.mpc.sample_imaginary_rollouts(
                actor='mpc', task_hor=self.task_hor, num_rollouts=2)
    ...
    def run_DAgger(self):
        """Learn parameterized policy with DAgger under the approximate model.
        """
        raise NotImplementedError
        # TODO finish implementing DAgger, but get behavior cloning to work first
        # self.mpc.sample_imaginary_rollouts(actor='policy') is not implemented yet
        # aggregating dataset in policy is not implemented yet
        ...
        for i in range(self.num_rollouts):
            ...
            # Generate imaginary rollouts with the parameterized policy
            obs_imagined, _ = self.mpc.sample_imaginary_rollouts(
                actor='policy', task_hor=self.task_hor, num_rollouts=2)

            # Label rollouts with actions from the controller
            labels_mpc = self.mpc.label(obs_imagined[:-1])

            # Train parameterized policy with DAgger
            metrics_policy = self.policy.train(obs_imagined[:-1], labels_mpc)
