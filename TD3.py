...
class TD3(object):
    def __init__(self, state_dim, action_dim, max_action):
        self.actor = Actor(state_dim, action_dim, max_action).to(TORCH_DEVICE)
        self.actor_target = Actor(state_dim, action_dim, max_action).to(TORCH_DEVICE)
        self.actor_target.load_state_dict(self.actor.state_dict())
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters())

        self.critic = Critic(state_dim, action_dim).to(TORCH_DEVICE)
        self.critic_target = Critic(state_dim, action_dim).to(TORCH_DEVICE)
    ...
    def act(self, state):
        state = torch.FloatTensor(state.reshape(1, -1)).to(TORCH_DEVICE)
        return self.actor(state).cpu().data.numpy().flatten()

    def act_parallel(self, state):
        state = torch.FloatTensor(state).to(TORCH_DEVICE)
    ...
    def train(self, replay_buffer, iterations, batch_size=100, discount=0.99, tau=0.005, policy_noise=0.2,
        ...
        for it in range(iterations):
            ...
            state = torch.FloatTensor(x).to(TORCH_DEVICE)
            action = torch.FloatTensor(u).to(TORCH_DEVICE)
            next_state = torch.FloatTensor(y).to(TORCH_DEVICE)
            notdone = torch.FloatTensor(1 - d).to(TORCH_DEVICE)
            reward = torch.FloatTensor(r).to(TORCH_DEVICE)

            # Select action according to policy and add clipped noise
            noise = torch.FloatTensor(u).data.normal_(0, policy_noise).to(TORCH_DEVICE)
